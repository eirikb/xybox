...
graphics = (function() {
  ...
  events.on('onload', function() {
    self.canvas = document.getElementById('gamepanel');
  ...
  events.on('tick', 2, function() {
    ...
    for (var i = 0; i < self.stage.getNumChildren(); i++) {
      var shape = self.stage.getChildAt(i);
      var x = shape.x + self.stage.x;
      var y = shape.y + self.stage.y;
      shape.visible = x >= 0 && x < self.canvas.width + shape.width && y >= 0 && y < self.canvas.height + shape.height;
    }
